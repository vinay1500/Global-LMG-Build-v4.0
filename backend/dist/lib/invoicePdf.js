...
const formatCurrency = (amount, currencyCode) => new Intl.NumberFormat('en-IN', {
...
const ensureSpace = (document, y, requiredHeight) => {
...
const drawLabelValue = (document, y, label, value) => {
    document
        .font('Helvetica')
        .fontSize(10)
        .fillColor('#6b7280')
...
export const renderInvoicePdf = async (invoice) => {
    ...
    for (const line of invoice.lines) {
        ...
        if (line.taxes.length > 0) {
            document
                ...
                .fontSize(8)
                .fillColor('#6b7280')
                .text(line.taxes.map((tax) => `${tax.name} (${tax.percent.toFixed(2)}%)`).join(' · '), 50, y + 2, { width: 320 });
            y = document.y + 6;
        }
        else {
            y += 8;
    ...
    if (invoice.installments.length > 0) {
        ...
        for (const installment of invoice.installments) {
            ...
            document
                ...
                .fontSize(10)
                .fillColor('#374151')
                .text(`Installment ${installment.installmentNo} · Due ${formatDate(installment.dueDate)} · ${installment.statusCode}`, 50, y, { width: 360 })
