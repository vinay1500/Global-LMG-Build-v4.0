...
const checksumMigration = (migration) => createHash('sha256').update(migration.statements.join('\n')).digest('hex');
...
const getAppliedMigrations = async (connection) => {
    const [rows] = await connection.query('SELECT id, checksum FROM schema_migrations ORDER BY id ASC');
...
const applyMigration = async (connection, migration) => {
    ...
    await connection.execute('INSERT INTO schema_migrations (id, description, checksum, executed_at) VALUES (?, ?, ?, ?)', [migration.id, migration.description, checksum, new Date()]);
};
            ...
            try {
                ...
                for (const migration of MIGRATIONS) {
                    ...
                    if (appliedChecksum && appliedChecksum !== checksum) {
                        throw new Error(`Migration checksum mismatch for ${migration.id}. Review schema history before continuing.`);
                    }
                    if (!appliedChecksum) {
