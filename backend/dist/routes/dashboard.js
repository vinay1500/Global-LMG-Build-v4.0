...
}).refine((value) => value.content.length > 0 || value.attachmentUploadIds.length > 0, {
...
const respondWithSnapshot = (response, snapshot) => {
    response.json(snapshot);
};
dashboardRouter.get('/dashboard', asyncHandler(async (request, response) => {
...
dashboardRouter.post('/dashboard/requests', asyncHandler(async (request, response) => {
    requireCsrf(request);
    const authenticatedUser = await requireAuthenticatedUser(request, response);
    const payload = dashboardRequestSchema.parse(request.body);
    const snapshot = await dashboardService.submitRequest(toDashboardUser(authenticatedUser), payload);
    respondWithSnapshot(response, snapshot);
}));
dashboardRouter.post('/dashboard/messages', asyncHandler(async (request, response) => {
    requireCsrf(request);
    const authenticatedUser = await requireAuthenticatedUser(request, response);
    const payload = dashboardMessageSchema.parse(request.body);
    const snapshot = await dashboardService.sendMessage(toDashboardUser(authenticatedUser), payload.threadId, payload.content, payload.attachmentUploadIds);
    respondWithSnapshot(response, snapshot);
}));
