...
const toPendingChallenge = (type, row) => {
    if (!row.expiresAt || !row.sentAt) {
        return undefined;
    }
    const normalizedProviderCode = type === 'phone'
        ? (row.providerCode ||
...
export class MysqlAuthStore {
    ...
    async getUserIdByPublicId(connection, publicId) {
        const row = await selectOne(connection, 'SELECT id FROM users WHERE public_id = ? LIMIT 1', [publicId]);
        return row?.id;
    }
    async upsertPrimaryClientAddress(connection, clientAccountId, country) {
        const timestamp = toMysqlDateTime(nowUtc());
        const existingPrimary = await selectOne(connection, `SELECT id
       FROM client_addresses
       WHERE client_account_id = ?
         AND archived_at IS NULL
         AND is_primary = 1
       ORDER BY id ASC
       LIMIT 1`, [clientAccountId]);
        if (existingPrimary?.id) {
            await connection.execute(`UPDATE client_addresses
         ...
         WHERE client_account_id = ?
           AND archived_at IS NULL
           AND is_primary = 1
           AND id <> ?`, [timestamp, timestamp, clientAccountId, existingPrimary.id]);
            return;
        }
        await connection.execute(`INSERT INTO client_addresses (
    ...
    async getOrCreateClientAccount(connection, userId, email, phone, fullName, country) {
        const existing = await selectOne(connection, `SELECT ca.id
       ...
       LIMIT 1`, [userId]);
        if (existing?.id) {
            const timestamp = toMysqlDateTime(nowUtc());
            await connection.execute(`UPDATE client_accounts
        ...
        const timestamp = toMysqlDateTime(nowUtc());
        const [result] = await connection.execute(`INSERT INTO client_accounts (
        ...
        const clientAccountId = Number(result.insertId);
        await connection.execute(`INSERT INTO client_account_contacts (
        client_account_id, user_id, contact_role_code, is_primary, is_billing,
        portal_access_enabled, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [clientAccountId, userId, 'primary', 1, 1, 1, timestamp, timestamp]);
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?)`, [userId, 1, 1, 1, 1, 0, timestamp]);
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [userId, 'client', null, timestamp, null, 1, timestamp, timestamp]);
        return clientAccountId;
    }
    async hydrateAccount(connection, whereClause, value) {
       ...
       LIMIT 1`, [value]);
        ...
        return {
            ...
            phone: row.phone || '',
            provider: (row.has_google ? 'google' : 'email'),
        };
    }
    async getFlowTokenIds(connection, hashedToken) {
        const row = await selectOne(connection, `SELECT email_token_id, phone_token_id, password_reset_token_id
       FROM auth_flows
    ...
    async deleteSessionByHashedToken(hashedToken) {
        await this.initialize();
        await withConnection(this.pool, (connection) => connection.execute('DELETE FROM user_sessions WHERE session_token_hash = ?', [hashedToken]));
    }
    ...
    async getAccountByEmail(email) {
        await this.initialize();
        return withConnection(this.pool, (connection) => this.hydrateAccount(connection, 'u.email = ?', normalizeEmail(email)));
    }
    async getAccountById(id) {
        await this.initialize();
        return withConnection(this.pool, (connection) => this.hydrateAccount(connection, 'u.public_id = ?', id));
    }
    async getAccountByPhone(phone) {
        ...
        await this.initialize();
        return withConnection(this.pool, (connection) => this.hydrateAccount(connection, 'u.phone = ?', normalizedPhone));
    }
    ...
    async saveChallenge(connection, userId, purposeCode, challenge) {
        if (!challenge) {
            return undefined;
        }
        const timestamp = toMysqlDateTime(nowUtc());
        if (challenge.type === 'email') {
            ...
            const [result] = await connection.execute(`INSERT INTO email_verification_tokens (
        ...
        if (challenge.type === 'phone') {
            const [result] = await connection.execute(`INSERT INTO phone_verification_tokens (
        ...
        const [result] = await connection.execute(`INSERT INTO password_reset_tokens (
    ...
    async saveAccount(account, options) {
        ...
        await withTransaction(this.pool, async (connection) => {
            ...
            if (!userId) {
                const existingByEmail = await selectOne(connection, 'SELECT id FROM users WHERE email = ? LIMIT 1', [normalizedEmail]);
          ...
          ON DUPLICATE KEY UPDATE
            ...
            must_rotate_password = VALUES(must_rotate_password)`, [userId, account.passwordHash, 'scrypt', timestamp, 0]);
            }
            await this.getOrCreateClientAccount(connection, userId, normalizedEmail, normalizedPhone || '', account.fullName, account.country);
            ...
            if (account.provider === 'google') {
                await connection.execute(`INSERT INTO user_oauth_accounts (
    ...
    async saveFlow(flow) {
        ...
        await withTransaction(this.pool, async (connection) => {
            ...
            const passwordResetTokenId = await this.saveChallenge(connection, userId, flow.purpose, flow.passwordResetChallenge);
            const timestamp = toMysqlDateTime(nowUtc());
            await connection.execute(`INSERT INTO auth_flows (
    ...
    async saveSession(session) {
        ...
        ON DUPLICATE KEY UPDATE
          ...
          updated_at = VALUES(updated_at)`, [
            ...
            await connection.execute('UPDATE users SET last_login_at = ?, updated_at = ? WHERE id = ?', [toMysqlDateTime(session.lastSeenAt), timestamp, userId]);
        });
    }
}
