...
const getRepository = async () => {
    if (!isMysqlConfigured) {
        throw serviceUnavailable('dashboard_storage_unavailable', 'Dashboard access requires a configured MySQL connection.');
    }
...
export const dashboardService = {
    ...
    async sendMessage(currentClient, threadId, content, attachmentUploadIds = []) {
        const repository = await getRepository();
        return repository.sendMessage(currentClient, threadId, content, attachmentUploadIds);
    },
    createEmptySnapshot(currentClient) {
        return createEmptyDashboardSnapshot(currentClient);
    },
};
