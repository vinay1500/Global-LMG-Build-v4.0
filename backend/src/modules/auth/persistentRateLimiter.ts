...
const getRetryAfterSeconds = (until: Date, now = Date.now()) =>
...
export const getPersistentRateLimitStatus = async (
  ...
  if (row.attempt_count >= input.maxAttempts) {
    return {
      allowed: false,
      retryAfterSeconds: getRetryAfterSeconds(resetsAt, now),
    };
  }
...
export const consumePersistentRateLimit = async (
  ...
  try {
    ...
    if (blockedUntil && blockedUntil.getTime() > nowMs) {
      ...
      return {
        allowed: false,
        retryAfterSeconds: getRetryAfterSeconds(blockedUntil, nowMs),
      };
    }
    ...
    if (currentCount >= input.maxAttempts) {
      ...
      return {
        allowed: false,
        retryAfterSeconds: getRetryAfterSeconds(retryUntil, nowMs),
      };
    }
