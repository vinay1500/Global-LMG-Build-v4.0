...
import type { PortalNotification } from './types.js';
...
const resolveMeta = (row: NotificationRow) => {
...
const resolveAction = (row: NotificationRow) => {
  if (row.thread_public_id) {
    return {
      actionLabel: 'Open Messages',
      actionTarget: 'messages' as const,
  ...
  if (row.document_public_id) {
    return {
      ...
      actionTarget: 'documents' as const,
      threadId: null,
    };
  }
  ...
  return {
    ...
    actionTarget: 'cases' as const,
    threadId: null,
  };
};

const toPortalNotification = (row: NotificationRow): PortalNotification => {
...
export class NotificationsRepository {
  ...
  public async listForUser(userPublicId: string) {
    ...
    return withConnection(this.pool, async (connection) => {
      ...
      const rows = await selectAll<NotificationRow>(
        connection,
        `SELECT
        ...
        [context.user_id, toMysqlDateTime(nowUtc())]
      );
  ...
  public async markRead(userPublicId: string, notificationPublicId: string) {
    await this.initialize();

    await withTransaction(this.pool, async (connection) => {
      const context = await this.resolvePortalUserContext(connection, userPublicId);
      const existing = await selectOne<RowDataPacket>(
        connection,
        `SELECT id
         FROM notifications
         WHERE public_id = ?
         ...
         LIMIT 1`,
        [notificationPublicId, context.user_id]
      );
      ...
      await connection.execute(
        `UPDATE notifications
         ...
         WHERE id = ?`,
        [timestamp, Number(existing.id)]
      );
    });
  }

  public async dismiss(userPublicId: string, notificationPublicId: string) {
    await this.initialize();
      ...
      await connection.execute(
        `UPDATE notifications
         SET is_read = 1,
             ...
             dismissed_at = ?
         WHERE id = ?`,
        [timestamp, timestamp, Number(existing.id)]
      );
    });
  }
}
