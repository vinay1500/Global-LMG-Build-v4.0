...
import type { StoredUploadRecord, StoredUploadRepository } from './types.js';

interface UploadIntentRow extends RowDataPacket {
  ...
  status_code: string;
  storage_driver_code: string;
  storage_key: string;
  stored_at: string | Date | null;
  thread_public_id: string | null;
}

const deriveRelatedEntity = (row: UploadIntentRow) => {
...
export class MysqlStoredUploadRepository implements StoredUploadRepository {
  ...
  public async getById(id: string) {
    ...
    return withConnection(this.pool, async (connection) => {
      const row = await selectOne<UploadIntentRow>(
        connection,
        `SELECT
           dui.public_id,
      ...
      return {
        ...
        status: row.status_code as StoredUploadRecord['status'],
        storageDriver: row.storage_driver_code as StoredUploadRecord['storageDriver'],
        storageKey: row.storage_key,
      } satisfies StoredUploadRecord;
    });
  }

  public async save(record: StoredUploadRecord) {
    await this.initialize();

    await withConnection(this.pool, async (connection) => {
      const ownerRow = await selectOne<RowDataPacket>(
        connection,
        `SELECT
           u.id AS user_id,
      ...
      const requestPublicId =
        record.relatedEntityType === 'request' ? record.relatedEntityId || null : null;
      const matterPublicId =
        record.relatedEntityType === 'matter' ? record.relatedEntityId || null : null;
      const invoicePublicId =
        record.relatedEntityType === 'invoice' ? record.relatedEntityId || null : null;
      const threadPublicId =
        record.relatedEntityType === 'thread' ? record.relatedEntityId || null : null;
