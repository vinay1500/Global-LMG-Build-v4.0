...
const requireAdminPermission = async (
  request: Parameters<typeof requireActor>[0],
  response: Parameters<typeof requireActor>[1],
...
adminRouter.get(
  '/admin/client-accounts/:clientAccountId',
  asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'client_account.view', [
      'ops_admin',
      'case_manager',
    ...
    response.json(
      await domainService.getClientAccountByPublicId(getRouteParam(request.params.clientAccountId))
    );
  })
);

adminRouter.get(
  '/admin/counsel-partners',
  asyncHandler(async (request, response) => {
...
adminRouter.get(
  '/admin/counsel-partners/:counselPartnerId',
  asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'counsel_partner.view', [
      'ops_admin',
      'case_manager',
    ...
    response.json(
      await domainService.getCounselPartnerByPublicId(getRouteParam(request.params.counselPartnerId))
    );
  })
);
...
adminRouter.post(
  '/admin/matters/:matterId/assignments',
  asyncHandler(async (request, response) => {
    requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'matter.update', [
      'ops_admin',
    ...
    response.status(201).json(
      await domainService.createMatterAssignment(
        actor.publicId,
        getRouteParam(request.params.matterId),
        payload
      )
...
adminRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    response
      .status(201)
      .json(
        await domainService.createEvent(
          actor.publicId,
          actor.roleCodes[0] || actor.actorTypeCode,
          payload
        )
...
adminRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    response
      .status(201)
      .json(
        await domainService.createRefund(
          actor.publicId,
          actor.roleCodes[0] || actor.actorTypeCode,
          payload
        )
