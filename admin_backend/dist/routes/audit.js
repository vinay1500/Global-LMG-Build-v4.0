...
auditRouter.get('/audit', asyncHandler(async (request, response) => {
    ...
    response.json(await adminAuditService.listAudit({
        actorName: typeof request.query.actorName === 'string' ? request.query.actorName : undefined,
        entityTableName: typeof request.query.entityTableName === 'string'
            ? request.query.entityTableName
            : undefined,
        limit: Number(request.query.limit || 50),
        search: typeof request.query.search === 'string' ? request.query.search : undefined,
        sourceModule: typeof request.query.sourceModule === 'string' ? request.query.sourceModule : undefined,
    }));
}));
auditRouter.get('/audit/download', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'audit.view', [
        'ops_admin',
        'case_manager',
        'billing_admin',
...
auditRouter.get('/audit/:auditId', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'audit.view', [
        'ops_admin',
        'case_manager',
        'billing_admin',
