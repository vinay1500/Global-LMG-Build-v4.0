...
rbacRouter.get('/rbac/roles', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response.json(await domainService.listRoles());
}));
rbacRouter.get('/rbac/permissions', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response.json(await domainService.listPermissions());
}));
rbacRouter.get('/rbac/users', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response.json(await domainService.listUsersWithRoles());
}));
rbacRouter.put('/rbac/users/:userId/roles', asyncHandler(async (request, response) => {
...
rbacRouter.post('/rbac/users', asyncHandler(async (request, response) => {
    await requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response
        .status(201)
...
rbacRouter.post('/rbac/users/:userId/reset-password', asyncHandler(async (request, response) => {
    await requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response.json(await domainService.resetAdminUserPassword(actor.publicId, getRouteParam(request.params.userId), resetAdminPasswordSchema.parse(request.body)));
}));
rbacRouter.patch('/rbac/users/:userId/access', asyncHandler(async (request, response) => {
    await requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'rbac.manage', ['ops_admin']);
    response.json(await domainService.updateAdminUserAccess(actor.publicId, getRouteParam(request.params.userId), updateAdminUserAccessSchema.parse(request.body)));
}));
