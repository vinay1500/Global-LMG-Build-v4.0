...
mattersRouter.get('/matters', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'matter.view', [
        'ops_admin',
        'case_manager',
        'management_viewer',
    ]);
    response.json(await adminMatterService.listMatters({
...
mattersRouter.get('/matters/:matterId', asyncHandler(async (request, response) => {
    await requireAdminPermission(request, response, 'matter.view', [
        'ops_admin',
        'case_manager',
        'management_viewer',
...
mattersRouter.patch('/matters/:matterId/stage', asyncHandler(async (request, response) => {
...
mattersRouter.post('/matters/:matterId/updates', asyncHandler(async (request, response) => {
    await requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'matter.update', [
        'ops_admin',
        'case_manager',
    ...
    response
        .status(201)
        .json(await adminMatterService.createMatterUpdate(actor.userId, actor.roleCodes[0] || actor.actorTypeCode, getRouteParam(request.params.matterId), request.body));
}));
mattersRouter.post('/matters/:matterId/internal-notes', asyncHandler(async (request, response) => {
    await requireCsrf(request);
    const actor = await requireAdminPermission(request, response, 'matter.update', [
        'ops_admin',
        'case_manager',
    ...
    response
        .status(201)
        .json(await adminMatterService.addInternalNote(actor.userId, actor.roleCodes[0] || actor.actorTypeCode, getRouteParam(request.params.matterId), request.body));
}));
