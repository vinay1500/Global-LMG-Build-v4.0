...
export const getUserAgent = (request) => request.header('user-agent') || null;
export const setAdminSessionCookies = (response, input) => {
...
export const refreshCsrfCookie = (response) => {
...
export const setCsrfCookie = (response, csrfToken) => {
    appendCookie(response, env.CSRF_COOKIE_NAME, csrfToken, cookieSecurity);
};
export const clearAdminSessionCookies = (response) => {
    clearCookie(response, env.SESSION_COOKIE_NAME, cookieSecurity);
    clearCookie(response, env.CSRF_COOKIE_NAME, cookieSecurity);
};
export const requireAdminPermission = async (request, response, permissionCode, allowedRoleCodes) => {
...
export const getCsrfTokenFromRequest = (request) => {
    const cookies = parseCookies(request.headers.cookie);
