...
export const adminRequestService = {
    async listRequests(options = {}) {
        return withConnection(getMysqlPool(), async (connection) => {
            ...
            if (options.search?.trim()) {
                ...
                clauses.push(`(sr.title LIKE ? ESCAPE '\\\\' OR sr.request_number LIKE ? ESCAPE '\\\\' OR ca.display_name LIKE ? ESCAPE '\\\\' OR sr.issue_summary LIKE ? ESCAPE '\\\\')`);
    ...
    async getRequest(requestPublicId) {
        return withConnection(getMysqlPool(), async (connection) => {
            const requestRow = await selectOne(connection, `SELECT
           sr.public_id,
           sr.request_number,
           sr.status_code,
           sr.title,
         ...
         WHERE sr.public_id = ?
         ORDER BY rs.sort_order ASC, s.name ASC`, [requestPublicId]);
            const quotes = await selectAll(connection, `SELECT
           pq.public_id,
           pq.version_no,
           pq.service_count,
         ...
         WHERE sr.public_id = ?
         ORDER BY pq.version_no DESC`, [requestPublicId]);
            const quoteLines = await selectAll(connection, `SELECT
         ...
         INNER JOIN service_requests sr
           ON sr.id = pq.service_request_id
         WHERE sr.public_id = ?
         ORDER BY pq.version_no DESC, pql.sort_order ASC`, [requestPublicId]);
         ...
         LEFT JOIN document_versions dv
          ...
          AND dv.is_current = 1
         WHERE sr.public_id = ?
         ORDER BY d.updated_at DESC`, [requestPublicId]);
            const statusHistory = await selectAll(connection, `SELECT
         ...
         WHERE sr.public_id = ?
         ORDER BY rsh.created_at DESC`, [requestPublicId]);
