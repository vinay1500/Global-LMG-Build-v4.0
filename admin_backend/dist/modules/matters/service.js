...
export const adminMatterService = {
    ...
    async listMatters(options = {}) {
        return withConnection(getMysqlPool(), async (connection) => {
            ...
            if (options.search?.trim()) {
                ...
                whereParts.push(`(m.title LIKE ? ESCAPE '\\\\' OR m.matter_number LIKE ? ESCAPE '\\\\' OR ca.display_name LIKE ? ESCAPE '\\\\' OR m.issue_summary LIKE ? ESCAPE '\\\\')`);
    ...
    async getMatterWorkspace(matterPublicId) {
        ...
        return withConnection(getMysqlPool(), async (connection) => {
         ...
         ORDER BY ct.last_message_at DESC`, [matterPublicId]);
            const notes = await selectAll(connection, `SELECT
           n.public_id,
           n.body_text,
           n.created_at,
           u.display_name
         ...
         ORDER BY n.created_at DESC`, [matterPublicId]);
            const events = await selectAll(connection, `SELECT public_id, title, status_code, scheduled_start_at
         FROM events
         WHERE matter_id = (SELECT id FROM matters WHERE public_id = ? LIMIT 1)
         ORDER BY scheduled_start_at DESC`, [matterPublicId]);
            return {
    ...
    async createMatterUpdate(actorUserId, actorRoleCode, matterPublicId, input) {
        ...
        return withTransaction(getMysqlPool(), async (connection) => {
            const matter = await selectOne(connection, 'SELECT id, client_account_id FROM matters WHERE public_id = ? LIMIT 1', [matterPublicId]);
            if (!matter?.id) {
                throw notFound('matter_not_found', 'Matter not found.');
            }
            const insert = await executeResult(connection, `INSERT INTO matter_updates (
         ...
         WHERE id = ?`, [timestamp, timestamp, Number(matter.id)]);
            if (payload.visibleToClient) {
                const recipients = await adminNotificationService.listClientRecipients(connection, Number(matter.client_account_id));
                await adminNotificationService.insertNotifications(connection, recipients.map((entry) => entry.userId), {
    ...
    async addInternalNote(actorUserId, actorRoleCode, matterPublicId, input) {
        const payload = createInternalNoteSchema.parse(input);
        const timestamp = toMysqlDateTime(nowUtc());
        return withTransaction(getMysqlPool(), async (connection) => {
            const matter = await selectOne(connection, 'SELECT id FROM matters WHERE public_id = ? LIMIT 1', [matterPublicId]);
            if (!matter?.id) {
                throw notFound('matter_not_found', 'Matter not found.');
            }
            const insert = await executeResult(connection, `INSERT INTO admin_internal_notes (
