...
export const adminAuditService = {
    async listAudit(options = {}) {
        return withConnection(getMysqlPool(), async (connection) => {
            ...
            if (options.search?.trim()) {
                ...
                clauses.push("(ae.action_label LIKE ? ESCAPE '\\\\' OR ae.summary_new_value LIKE ? ESCAPE '\\\\' OR ae.summary_old_value LIKE ? ESCAPE '\\\\')");
    ...
    async getAuditDetail(auditPublicId) {
        return withConnection(getMysqlPool(), async (connection) => {
            const detail = await selectAll(connection, `SELECT
           ae.public_id,
           ae.entity_table_name,
           ae.entity_pk,
           ae.action_code,
         ...
         LEFT JOIN users actor
           ON actor.id = ae.actor_user_id
         WHERE ae.public_id = ?
         LIMIT 1`, [auditPublicId]);
            const row = detail[0];
            if (!row?.public_id) {
                return null;
            }
            const changes = await selectAll(connection, `SELECT id, field_name, old_value_text, new_value_text
         FROM audit_event_changes
         WHERE audit_event_id = (
           SELECT id FROM audit_events WHERE public_id = ? LIMIT 1
         )
         ORDER BY id ASC`, [auditPublicId]);
            return {
                actionCode: row.action_code,
                actionLabel: row.action_label,
                actorName: row.actor_name,
