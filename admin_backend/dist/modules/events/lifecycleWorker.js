...
const buildLifecycleEmail = (input) => {
    const intro = input.actionLabel === 'scheduled'
    ...
    const meetingLine = input.modeCode === 'video'
        ? input.joinUrl
...
const listInternalRecipientsForEvent = async (connection, eventId) => {
    const rows = await selectAll(connection, `SELECT DISTINCT
     ...
     WHERE ep.event_id = ?
       ...
       AND u.archived_at IS NULL`, [eventId]);
    return rows.map((row) => ({
        email: row.email,
        userId: Number(row.user_id),
    }));
};
const loadEventContext = async (connection, eventId) => {
    const event = await selectOne(connection, `SELECT
       id,
       public_id,
       client_account_id,
       matter_id,
...
const syncCalendarState = async (payload, event, attendees) => {
...
export const processEventLifecycleJob = async (payload) => {
    const eventContext = await withConnection(getMysqlPool(), async (connection) => loadEventContext(connection, payload.eventId));
    if (!eventContext) {
        return;
    }
    ...
    const calendarResult = await syncCalendarState(payload, event, audience.map((recipient) => ({ email: recipient.email })));
    ...
    await withTransaction(getMysqlPool(), async (connection) => {
        ...
        await executeResult(connection, `UPDATE events
       ...
       WHERE id = ?`, [
        ...
        const notificationBody = payload.actionLabel === 'scheduled'
        ...
        if (clientRecipients.length > 0) {
            await adminNotificationService.insertNotifications(connection, clientRecipients.map((recipient) => recipient.userId), {
        ...
        if (internalRecipients.length > 0) {
            await adminNotificationService.insertNotifications(connection, internalRecipients.map((recipient) => recipient.userId), {
