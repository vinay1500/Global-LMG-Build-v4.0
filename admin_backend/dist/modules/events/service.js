...
const loadClientParticipants = async (connection, clientAccountId) => {
...
const loadInternalParticipants = async (connection, actorUserId, participants) => {
    const userIds = new Set([actorUserId]);
    for (const participant of participants) {
        if (!participant.internalUserId) {
            continue;
        }
        const user = await selectOne(connection, 'SELECT id FROM users WHERE public_id = ? LIMIT 1', [participant.internalUserId]);
        if (user?.id) {
...
const resolveMatterForClient = async (connection, clientAccountId, matterPublicId) => {
    if (!matterPublicId) {
        return null;
    }
    const matter = await selectOne(connection, `SELECT id
     FROM matters
     WHERE public_id = ?
       AND client_account_id = ?
       AND archived_at IS NULL
     LIMIT 1`, [matterPublicId, clientAccountId]);
    if (!matter?.id) {
        throw notFound('matter_not_found', 'Matter was not found for the selected client account.');
    }
    return Number(matter.id);
};
const rebuildReminderRows = async (connection, eventId, recipientUserIds, scheduledStartAt, enabled) => {
    await connection.execute(`DELETE FROM event_reminders
     WHERE event_id = ?
       AND sent_at IS NULL`, [eventId]);
    ...
    for (const recipientUserId of uniqueRecipientUserIds) {
        for (const offsetMinutes of env.MEETING_REMINDER_OFFSETS) {
            ...
            await connection.execute(`INSERT INTO event_reminders (
          event_id,
          recipient_user_id,
          channel_code,
          scheduled_at,
...
const replaceEventParticipants = async (connection, eventId, clientRecipients, adminRecipients, timestamp) => {
    ...
    for (const recipient of clientRecipients) {
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [eventId, 'client', null, recipient.userId, null, 'pending', 'scheduled', null, null, timestamp]);
    }
    for (const recipient of adminRecipients) {
        await connection.execute(`INSERT INTO event_participants (
        event_id, participant_role_code, internal_user_id, client_contact_user_id, counsel_partner_id,
        rsvp_status_code, attendance_status_code, joined_at, left_at, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [eventId, 'internal_admin', recipient.userId, null, null, 'accepted', 'scheduled', null, null, timestamp]);
    }
};
const enqueueLifecycleJob = async (connection, payload, dedupeKey) => {
...
export const adminEventService = {
    ...
    async getEvent(eventPublicId) {
        const detail = await domainService.getEventByPublicId(eventPublicId);
        return withConnection(getMysqlPool(), async (connection) => {
            const reminders = await selectAll(connection, `SELECT
           id,
           channel_code,
           scheduled_at,
           sent_at,
         ...
         ORDER BY scheduled_at ASC`, [eventPublicId]);
            return {
                ...detail,
    ...
    async createEvent(actorUserId, actorRoleCode, input) {
        const payload = eventMutationSchema.parse(input);
        const result = await withTransaction(getMysqlPool(), async (connection) => {
            const clientAccount = await selectOne(connection, 'SELECT id FROM client_accounts WHERE public_id = ? LIMIT 1', [payload.clientAccountId]);
            if (!clientAccount?.id) {
            ...
            const clientRecipients = payload.clientVisibleFlag
                ...
                : [];
            const adminRecipients = await loadInternalParticipants(connection, actorUserId, payload.participants || []);
            const timestamp = toMysqlDateTime(nowUtc());
            const insert = await executeResult(connection, `INSERT INTO events (
          ...
          cancelled_by_user_id, created_at, updated_at, cancelled_at
            ...
            await rebuildReminderRows(connection, eventId, [...clientRecipients, ...adminRecipients].map((recipient) => recipient.userId), payload.scheduledStartAt, ['upcoming', 'rescheduled'].includes(payload.statusCode));
            ...
            const row = await selectOne(connection, 'SELECT public_id FROM events WHERE id = ? LIMIT 1', [eventId]);
            const eventPublicId = String(row?.public_id || '');
            await enqueueLifecycleJob(connection, {
    ...
    async updateEvent(actorUserId, actorRoleCode, eventPublicId, input) {
        const payload = eventMutationSchema.parse(input);
        const result = await withTransaction(getMysqlPool(), async (connection) => {
            const eventRow = await selectOne(connection, `SELECT id, client_account_id, matter_id, external_meeting_id, public_id
         FROM events
         WHERE public_id = ?
         LIMIT 1`, [eventPublicId]);
            ...
            const adminRecipients = await loadInternalParticipants(connection, actorUserId, payload.participants || []);
            const timestamp = toMysqlDateTime(nowUtc());
            await connection.execute(`UPDATE events
         ...
         WHERE id = ?`, [
            ...
            await replaceEventParticipants(connection, Number(eventRow.id), clientRecipients, adminRecipients, timestamp);
            await rebuildReminderRows(connection, Number(eventRow.id), [...clientRecipients, ...adminRecipients].map((recipient) => recipient.userId), payload.scheduledStartAt, ['upcoming', 'rescheduled'].includes(payload.statusCode));
            await adminNotificationService.insertAuditEvent(connection, {
    ...
    async cancelEvent(actorUserId, actorRoleCode, eventPublicId, input) {
        const payload = cancelEventSchema.parse(input);
        const result = await withTransaction(getMysqlPool(), async (connection) => {
            const eventRow = await selectOne(connection, `SELECT id, public_id, external_meeting_id
         FROM events
         WHERE public_id = ?
         LIMIT 1`, [eventPublicId]);
            if (!eventRow?.id) {
         ...
         SET status_code = 'cancelled',
             ...
             updated_at = ?
         WHERE id = ?`, [timestamp, actorUserId, timestamp, Number(eventRow.id)]);
         ...
         WHERE event_id = ?
           AND sent_at IS NULL`, [timestamp, payload.reasonText || 'cancelled', Number(eventRow.id)]);
            await adminNotificationService.insertAuditEvent(connection, {
