...
const buildRetryAt = (attemptCount) => {
...
export const adminJobQueue = {
    async enqueueInTransaction(connection, input) {
        ...
        await executeResult(connection, `INSERT INTO admin_async_jobs (
        public_id,
        type_code,
        dedupe_key,
        status_code,
    ...
    async claimDueJobs(limit = 20) {
        return withTransaction(getMysqlPool(), async (connection) => {
         ...
         FOR UPDATE SKIP LOCKED`, [limit]);
            ...
            await connection.execute(`UPDATE admin_async_jobs
         SET status_code = 'processing',
             ...
             updated_at = ?
         WHERE id IN (${rows.map(() => '?').join(', ')})`, [leaseToken, leaseExpiresAt, claimedAt, claimedAt, ...rows.map((row) => row.id)]);
            return rows.map((row) => ({
    ...
    async completeJob(jobId, leaseToken) {
        return withTransaction(getMysqlPool(), async (connection) => {
            const timestamp = toMysqlDateTime(nowUtc());
            await executeResult(connection, `UPDATE admin_async_jobs
         SET status_code = 'completed',
             ...
             updated_at = ?
         WHERE id = ?
           AND lease_token = ?`, [timestamp, timestamp, jobId, leaseToken]);
        });
    },
