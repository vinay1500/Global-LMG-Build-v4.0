const informationSchemaExists = async (connection, query, values) => {
    const [rows] = await connection.query(query, values);
    return Number(rows[0]?.count_value || 0) > 0;
};
const columnExists = async (connection, tableName, columnName) => informationSchemaExists(connection, `SELECT COUNT(*) AS count_value
     FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME = ?
       AND COLUMN_NAME = ?`, [tableName, columnName]);
const indexExists = async (connection, tableName, indexName) => informationSchemaExists(connection, `SELECT COUNT(*) AS count_value
     FROM information_schema.STATISTICS
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME = ?
       AND INDEX_NAME = ?`, [tableName, indexName]);
export const NORMALIZED_MIGRATIONS = [
    {
    ...
    },
    {
        ...
        description: 'Archive duplicate active primary client addresses and add lookup support for the primary-address upsert path.',
        statements: [
    ...
    },
    {
        ...
        description: 'Allow phone verification tokens to store Twilio-native verification metadata instead of custom OTP hashes.',
        statements: [
    ...
    {
        id: '012-admin-ops-tables',
        description: 'Create admin-only task, note, and view tables for the admin platform.',
        statements: [
        ...
        archived_at DATETIME(6) NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_tasks_public_id (public_id),
        INDEX idx_admin_tasks_status_due (status_code, due_at),
        INDEX idx_admin_tasks_client (client_account_id),
        INDEX idx_admin_tasks_matter (matter_id),
        CONSTRAINT fk_admin_tasks_client FOREIGN KEY (client_account_id)
          REFERENCES client_accounts (id)
          ON UPDATE CASCADE
          ON DELETE SET NULL,
        CONSTRAINT fk_admin_tasks_matter FOREIGN KEY (matter_id)
          REFERENCES matters (id)
          ON UPDATE CASCADE
          ON DELETE SET NULL,
        CONSTRAINT fk_admin_tasks_created_by FOREIGN KEY (created_by_user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE RESTRICT
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
        ...
        status_code VARCHAR(32) NOT NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_task_assignees_active (task_id, user_id),
        CONSTRAINT fk_admin_task_assignees_task FOREIGN KEY (task_id)
          REFERENCES admin_tasks (id)
          ON UPDATE CASCADE
          ON DELETE CASCADE,
        CONSTRAINT fk_admin_task_assignees_user FOREIGN KEY (user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE RESTRICT,
        CONSTRAINT fk_admin_task_assignees_assigned_by FOREIGN KEY (assigned_by_user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE RESTRICT
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
        ...
        updated_at DATETIME(6) NOT NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_task_comments_public_id (public_id),
        INDEX idx_admin_task_comments_task (task_id, created_at),
        CONSTRAINT fk_admin_task_comments_task FOREIGN KEY (task_id)
          REFERENCES admin_tasks (id)
          ON UPDATE CASCADE
          ON DELETE CASCADE,
        CONSTRAINT fk_admin_task_comments_created_by FOREIGN KEY (created_by_user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE RESTRICT
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
        ...
        updated_at DATETIME(6) NOT NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_internal_notes_public_id (public_id),
        INDEX idx_admin_internal_notes_entity (entity_type_code, entity_public_id, created_at),
        CONSTRAINT fk_admin_internal_notes_matter FOREIGN KEY (matter_id)
          REFERENCES matters (id)
          ON UPDATE CASCADE
          ON DELETE SET NULL,
        CONSTRAINT fk_admin_internal_notes_client FOREIGN KEY (client_account_id)
          REFERENCES client_accounts (id)
          ON UPDATE CASCADE
          ON DELETE SET NULL,
        CONSTRAINT fk_admin_internal_notes_created_by FOREIGN KEY (created_by_user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE RESTRICT
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
        ...
        updated_at DATETIME(6) NOT NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_saved_views_public_id (public_id),
        INDEX idx_admin_saved_views_user_module (user_id, module_code),
        CONSTRAINT fk_admin_saved_views_user FOREIGN KEY (user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
        ...
        user_id BIGINT UNSIGNED NOT NULL,
        layout_payload JSON NOT NULL,
        updated_at DATETIME(6) NOT NULL,
        PRIMARY KEY (user_id),
        CONSTRAINT fk_admin_dashboard_preferences_user FOREIGN KEY (user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`,
            `CREATE TABLE IF NOT EXISTS admin_activity_bookmarks (
        id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        public_id CHAR(26) NOT NULL,
        user_id BIGINT UNSIGNED NOT NULL,
        ...
        created_at DATETIME(6) NOT NULL,
        PRIMARY KEY (id),
        UNIQUE KEY uq_admin_activity_bookmarks_public_id (public_id),
        INDEX idx_admin_activity_bookmarks_user (user_id, created_at),
        CONSTRAINT fk_admin_activity_bookmarks_user FOREIGN KEY (user_id)
          REFERENCES users (id)
          ON UPDATE CASCADE
          ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`
    ...
    {
        id: '013-admin-async-jobs-and-reminder-leases',
        description: 'Add durable admin async jobs and lease-based reminder processing.',
        checksumSource: '013-admin-async-jobs-and-reminder-leases-v2',
        run: async (connection) => {
            if (!(await columnExists(connection, 'event_reminders', 'attempt_count'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD COLUMN attempt_count INT UNSIGNED NOT NULL DEFAULT 0`);
            }
            if (!(await columnExists(connection, 'event_reminders', 'lease_token'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD COLUMN lease_token CHAR(26) NULL`);
            }
            if (!(await columnExists(connection, 'event_reminders', 'claimed_at'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD COLUMN claimed_at DATETIME(6) NULL`);
            }
            if (!(await columnExists(connection, 'event_reminders', 'lease_expires_at'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD COLUMN lease_expires_at DATETIME(6) NULL`);
            }
            if (!(await columnExists(connection, 'event_reminders', 'provider_reference'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD COLUMN provider_reference VARCHAR(255) NULL`);
            }
            if (!(await indexExists(connection, 'event_reminders', 'idx_event_reminders_due_claim'))) {
                await connection.query(`ALTER TABLE event_reminders
           ADD INDEX idx_event_reminders_due_claim (
            ...
            await connection.query(`CREATE TABLE IF NOT EXISTS admin_async_jobs (
          id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
          public_id CHAR(26) NOT NULL,
          ...
          updated_at DATETIME(6) NOT NULL,
          PRIMARY KEY (id),
          UNIQUE KEY uq_admin_async_jobs_public_id (public_id),
          UNIQUE KEY uq_admin_async_jobs_dedupe_key (dedupe_key),
          INDEX idx_admin_async_jobs_due_claim (
            status_code,
            available_at,
            lease_expires_at,
            completed_at
