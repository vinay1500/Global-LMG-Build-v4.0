...
notificationsRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json(
      await withConnection(getMysqlPool(), (connection) =>
        adminNotificationService.listNotificationHistory(connection, {
          clientAccountId:
            typeof request.query.clientAccountId === 'string'
              ? request.query.clientAccountId
              : undefined,
          dismissed:
            typeof request.query.dismissed === 'string'
              ? request.query.dismissed === 'true'
              : undefined,
          isRead:
            typeof request.query.isRead === 'string'
              ? request.query.isRead === 'true'
              : undefined,
          limit: Number(request.query.limit || 40),
          notificationTypeCode:
            typeof request.query.notificationTypeCode === 'string'
              ? request.query.notificationTypeCode
              : undefined,
          offset: Number(request.query.offset || 0),
          recipientUserId:
            typeof request.query.recipientUserId === 'string'
