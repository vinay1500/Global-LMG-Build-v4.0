...
const isSafeInlinePreviewMimeType = (mimeType: string) =>
  ['application/pdf', 'image/png', 'image/jpeg', 'image/webp', 'text/plain'].includes(
...
documentsRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json(
      await adminDocumentService.listDocuments({
...
documentsRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    const result = await adminDocumentService.previewDocument(
      actor.publicId,
    ...
    if (!isSafeInlinePreviewMimeType(result.mimeType)) {
      throw badRequest(
        ...
        'This file type does not support safe inline preview. Download it instead.'
      );
    }
...
documentsRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    const result = await adminDocumentService.downloadDocument(
      actor.publicId,
      getRouteParam(request.params.documentId),
      {
        ipAddress: request.ip,
...
documentsRouter.patch(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json(
      await adminDocumentService.updateVisibility(
        getRouteParam(request.params.documentId),
        request.body
      )
    );
