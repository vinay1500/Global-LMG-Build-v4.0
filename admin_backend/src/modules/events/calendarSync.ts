...
export interface CalendarEventMutationInput {
  ...
  timeZone: string;
  title: string;
  attendees: CalendarParticipantInput[];
}
...
export interface CalendarEventMutationResult {
  ...
  joinUrl: string | null;
  providerCode: string;
  syncStatusCode: 'disabled' | 'synced';
}
...
const resolveJwtClient = () => {
  ...
  if (
    ...
    throw serviceUnavailable(
      'calendar_provider_misconfigured',
      'Google Calendar provider is not fully configured.'
    );
  }
...
const extractConferenceUri = (body: any) => {
...
const callCalendarApi = async (
  ...
  if (!token) {
    return {
      ...
      joinUrl: null,
      providerCode: 'none',
      syncStatusCode: 'disabled' as const,
    };
  }
  ...
  if (!response.ok) {
    ...
    throw serviceUnavailable(
      'calendar_provider_failed',
      `Google Calendar rejected the request with status ${response.status}: ${errorBody}`
    );
  }
  ...
  return {
    ...
    syncStatusCode: 'synced' as const,
  };
};

const buildCalendarPayload = (input: CalendarEventMutationInput) => ({
...
export const adminCalendarSyncService = {
  async createMeeting(input: CalendarEventMutationInput): Promise<CalendarEventMutationResult> {
    return callCalendarApi(
      '/events?conferenceDataVersion=1&sendUpdates=none',
      'POST',
      buildCalendarPayload(input)
    );
  },
  ...
  async updateMeeting(
    ...
    return callCalendarApi(
      `/events/${encodeURIComponent(externalEventId)}?conferenceDataVersion=1&sendUpdates=none`,
      'PATCH',
      buildCalendarPayload(input)
    );
  },

  async cancelMeeting(externalEventId: string) {
    const token = await getBearerToken();
    ...
    if (!response.ok) {
      ...
      throw serviceUnavailable(
        'calendar_provider_failed',
        `Google Calendar rejected the cancellation with status ${response.status}: ${errorBody}`
      );
    }
