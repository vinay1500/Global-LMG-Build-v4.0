...
let repositoryPromise: Promise<AdminAuthRepository> | null = null;
...
const hashSessionToken = (rawSessionToken: string) =>
  hashOpaqueValue(rawSessionToken, env.AUTH_SESSION_SECRET);
const hashCsrfToken = (rawCsrfToken: string) => hashOpaqueValue(rawCsrfToken, env.AUTH_SESSION_SECRET);

export const authService = {
  async signIn(
    input: AdminAuthSignInInput,
    options: {
    ...
    if (!roleCodes.some((roleCode) => INTERNAL_ROLE_CODES.has(roleCode))) {
      ...
      throw forbidden(
        'admin_role_required',
        'This account is not assigned an internal admin role.'
      );
    }
  ...
  async getSession(rawSessionToken: string | undefined): Promise<SessionResolution> {
    if (!rawSessionToken) {
      return { user: null };
    }
    ...
    if (!roleCodes.some((roleCode) => INTERNAL_ROLE_CODES.has(roleCode))) {
      throw conflict(
        'admin_session_role_missing',
        'This session belongs to a user without an active admin role.'
      );
    }
  ...
  async validateCsrf(rawSessionToken: string, rawCsrfToken: string) {
    const repository = await getRepository();
    const session = await repository.resolveSession(hashSessionToken(rawSessionToken));
  ...
  async rotateCsrf(rawSessionToken: string) {
    const repository = await getRepository();
    const session = await repository.resolveSession(hashSessionToken(rawSessionToken));
