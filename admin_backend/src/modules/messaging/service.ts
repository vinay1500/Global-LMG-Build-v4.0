...
interface ThreadListRow extends RowDataPacket {
...
interface MessageRow extends RowDataPacket {
  ...
  public_id: string;
  sender_system_code: string | null;
  sent_at: string;
}
...
interface AvailableAttachmentRow extends RowDataPacket {
...
export const adminMessagingService = {
  ...
  async listThreads(options: {
    ...
    return withTransaction(getMysqlPool(), async (connection) => {
      ...
      const rows = await selectAll<ThreadListRow>(
        connection,
        `SELECT
           ct.public_id,
           ct.thread_number,
  ...
  async getThread(threadPublicId: string) {
    return withTransaction(getMysqlPool(), async (connection) => {
      const thread = await selectOne<ThreadListRow>(
        connection,
        `SELECT
           ct.id,
      ...
      const attachments = await selectAll<AttachmentRow>(
        connection,
        `SELECT
      ...
      const availableAttachments = await selectAll<AvailableAttachmentRow>(
        connection,
        `SELECT
           d.public_id AS document_public_id,
           d.title,
         ...
         LIMIT 24`,
        [Number(thread.client_account_id)]
      );

      const attachmentMap = new Map<string, Array<{ documentId: string; originalFileName: string }>>();
  ...
  async sendReply(
    ...
    return withTransaction(getMysqlPool(), async (connection) => {
      const thread = await selectOne<RowDataPacket>(
        connection,
        `SELECT id, client_account_id, matter_id
      ...
      const participant = await selectOne<RowDataPacket>(
        connection,
        `SELECT id
      ...
      if (!participant?.id) {
        await connection.execute(
          ...
          [Number(thread.id), 'internal_admin', actorUserId, null, null, 1, timestamp, null, null, null]
        );
      }
      ...
      for (const documentPublicId of payload.attachmentDocumentIds || []) {
        const documentVersion = await selectOne<RowDataPacket>(
          connection,
          `SELECT dv.id
           FROM documents d
           INNER JOIN document_versions dv
           ...
           WHERE d.public_id = ?
             ...
             AND d.archived_at IS NULL
           LIMIT 1`,
          [documentPublicId, Number(thread.client_account_id)]
        );
        ...
        await connection.execute(
          ...
          [messageId, Number(documentVersion.id), 0, timestamp]
        );
      }
      ...
      await connection.execute(
        `UPDATE conversation_threads
         ...
         WHERE id = ?`,
        [timestamp, timestamp, Number(thread.id)]
      );

      await adminNotificationService.insertNotifications(
        connection,
        (
          await adminNotificationService.listClientRecipients(connection, Number(thread.client_account_id))
