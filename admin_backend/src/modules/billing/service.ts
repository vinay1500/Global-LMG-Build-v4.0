...
interface BillingClientRow extends RowDataPacket {
...
interface BillingContactRow extends RowDataPacket {
  email: string;
  is_billing: number;
  is_primary: number;
  name: string;
...
interface BillingAddressRow extends RowDataPacket {
  city: string;
  country_code: string;
  line1: string;
  line2: string | null;
...
interface ManualPaymentAllocationRow extends RowDataPacket {
  ...
  due_date: string;
  invoice_id: number;
  invoice_installment_id: number | null;
...
const buildInvoiceEmailHtml = (input: {
  ...
  [
    ...
    `<p>Total: <strong>${escapeHtml(String(input.totalAmount))} ${escapeHtml(
      input.currencyCode
    )}</strong><br />Amount due: <strong>${escapeHtml(String(input.amountDue))} ${escapeHtml(
      input.currencyCode
    )}</strong><br />Due date: <strong>${escapeHtml(input.dueDate)}</strong></p>`,
    input.messageText
...
const requireBillingSnapshot = (snapshot: {
  ...
  if (missing.length > 0) {
    throw badRequest(
      'invoice_billing_snapshot_incomplete',
      `Billing snapshot is missing: ${missing.join(', ')}.`
    );
  }
...
export const adminBillingService = {
  ...
  async getInvoice(invoicePublicId: string) {
    return domainService.getInvoiceByPublicId(invoicePublicId);
  },

  async getInvoicePdf(invoicePublicId: string) {
    ...
    const created = await withTransaction(getMysqlPool(), async (connection) => {
      const client = await selectOne<BillingClientRow>(
        ...
        `SELECT
         ...
         LIMIT 1`,
        [payload.matterId, payload.clientAccountId]
      );

      if (!client?.client_account_id || !client?.matter_id) {
        throw notFound(
          'invoice_client_or_matter_not_found',
          'The selected client or matter could not be resolved together.'
        );
      }

      const contact = await selectOne<BillingContactRow>(
        connection,
        `SELECT
      ...
      const address = await selectOne<BillingAddressRow>(
        connection,
        `SELECT
           line1,
           line2,
      ...
      const billingSnapshot = requireBillingSnapshot({
        ...
        billingEmail:
          payload.billingSnapshot?.billingEmail ||
          payload.recipientEmail ||
          contact?.email ||
          client.primary_email,
        billingName:
          payload.billingSnapshot?.billingName ||
          contact?.name ||
          client.display_name ||
          client.legal_name,
        billingPhone:
          payload.billingSnapshot?.billingPhone || contact?.phone || client.primary_phone,
      ...
      const normalizedLines = payload.lines.map((line, index) => {
        ...
        if (discountAmount > subtotal) {
          throw badRequest(
            'invoice_line_discount_invalid',
            `Line ${index + 1} has a discount larger than its subtotal.`
          );
        }
      ...
      const subtotalAmount = roundMoney(
        normalizedLines.reduce((sum, line) => sum + line.subtotal, 0)
      );
      ...
      const packageResult = await executeResult(
        connection,
      ...
      const invoiceResult = await executeResult(
        connection,
      ...
      for (const [index, line] of normalizedLines.entries()) {
        const lineResult = await executeResult(
          connection,
      ...
      await connection.execute(
        ...
        [invoiceId, 1, payload.dueDate, totalAmount, 0, totalAmount, 'pending', null, timestamp]
      );
        ...
        await executeResult(
          ...
          `UPDATE invoices
           SET status_code = 'sent',
               updated_at = ?
           WHERE public_id = ?`,
          [toMysqlDateTime(nowUtc()), created.invoiceId]
        );
      });
    }
  ...
  async sendInvoiceEmail(
    ...
    if (!recipientEmail) {
      throw badRequest(
        ...
        'This invoice does not have a billing email. Provide a recipient email to send it.'
      );
    }
      ...
      await executeResult(
        ...
        `UPDATE invoices
         SET status_code = CASE WHEN status_code = 'paid' THEN status_code ELSE 'sent' END,
             updated_at = ?
         WHERE public_id = ?`,
        [toMysqlDateTime(nowUtc()), invoicePublicId]
      );
    });
  ...
  async recordManualPayment(
    actorUserId: number,
    actorRoleCode: string,
    input: z.infer<typeof recordManualPaymentSchema>
  ) {
    const payload = recordManualPaymentSchema.parse(input);
    const amountApplied = roundMoney(
      payload.allocations.reduce((sum, allocation) => sum + allocation.amountApplied, 0)
    );

    if (roundMoney(payload.amount) !== amountApplied) {
      throw badRequest(
        'manual_payment_amount_mismatch',
        'Payment amount must match the total allocated amount.'
      );
    }

    return withTransaction(getMysqlPool(), async (connection) => {
      const timestamp = toMysqlDateTime(payload.initiatedAt || nowUtc());
      const client = await selectOne<RowDataPacket>(
        connection,
        'SELECT id FROM client_accounts WHERE public_id = ? AND archived_at IS NULL LIMIT 1',
      ...
      for (const allocation of payload.allocations) {
        const resolved = await selectOne<ManualPaymentAllocationRow>(
          connection,
          `SELECT
             i.id AS invoice_id,
             i.client_account_id,
           ...
           LIMIT 1`,
          [allocation.installmentId || null, allocation.invoiceId]
        );
        ...
        if (Number(resolved.client_account_id) !== Number(client.id)) {
          throw conflict(
            'manual_payment_client_mismatch',
            'Every allocated invoice must belong to the selected client account.'
          );
        }
        ...
        if (resolved.currency_code !== payload.currencyCode.toUpperCase()) {
          throw conflict(
            ...
            'Allocated invoices must use the same currency as the payment.'
          );
        }

        if (Number(allocation.amountApplied) > roundMoney(Number(resolved.amount_due || 0))) {
          throw conflict(
            'manual_payment_exceeds_invoice_due',
            'Allocated amount exceeds the amount due on one of the invoices.'
          );
        }
      ...
      const paymentInsert = await executeResult(
        connection,
      ...
      const touchedMatterIds = new Set<number>();
      ...
      for (const allocation of allocationRows) {
        await connection.execute(
          ...
          [paymentId, allocation.invoiceId, allocation.installmentId, allocation.amountApplied, timestamp]
        );
      ...
      for (const matterId of touchedMatterIds) {
        const totals = await selectOne<RowDataPacket>(
          connection,
          `SELECT
  ...
  async getOverview() {
    return withConnection(getMysqlPool(), async (connection) => {
      const [outstanding, overdue, paidInvoices, refunds] = await Promise.all([
        selectOne<InvoiceCountRow>(
          connection,
          'SELECT COUNT(*) AS count_value FROM invoices WHERE archived_at IS NULL AND amount_due > 0'
        ),
        selectOne<InvoiceCountRow>(
          connection,
