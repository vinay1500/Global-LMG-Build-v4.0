...
interface ClientListRow extends RowDataPacket {
...
interface LegalAcceptanceRow extends RowDataPacket {
...
interface SessionAccessRow extends RowDataPacket {
  created_at: string;
...
export const adminClientService = {
  async listClients(options: {
    ...
    return withConnection(getMysqlPool(), async (connection) => {
      ...
      const rows = await selectAll<ClientListRow>(
        connection,
        `SELECT
           ca.public_id,
           ca.client_code,
  ...
  async getClient360(clientAccountPublicId: string) {
    const base = await domainService.getClientAccountByPublicId(clientAccountPublicId);

    return withConnection(getMysqlPool(), async (connection) => {
      const account = await selectOne<RowDataPacket>(
        connection,
      ...
      const portalUsers = await selectAll<PortalUserRow>(
        connection,
        `SELECT
           u.id AS user_id,
           u.public_id,
      ...
      const legalAcceptances =
        portalUserIds.length > 0
          ? await selectAll<LegalAcceptanceRow>(
              connection,
              `SELECT
                 user_id,
                 public_id,
      ...
      const activeSessions =
        portalUserIds.length > 0
          ? await selectAll<SessionAccessRow>(
              connection,
              `SELECT
                 user_id,
                 public_id,
      ...
      const securityEvents =
        portalUserIds.length > 0
          ? await selectAll<SecurityEventRow>(
              connection,
              `SELECT
                 user_id,
                 public_id,
      ...
      const internalNotes = await selectAll<NoteRow>(
        connection,
        `SELECT
           n.public_id,
           n.body_text,
      ...
      const activity = await selectAll<AuditRow>(
        connection,
        `SELECT
           ae.public_id,
           ae.action_label,
      ...
      const preferenceByUserId = new Map<number, PreferenceRow>();
      for (const entry of preferences) {
        preferenceByUserId.set(Number(entry.user_id), entry);
      }

      const legalAcceptancesByUserId = new Map<number, LegalAcceptanceRow[]>();
      ...
      const sessionsByUserId = new Map<number, SessionAccessRow[]>();
      ...
      const securityEventsByUserId = new Map<number, SecurityEventRow[]>();
      ...
      return {
        ...
        portalUsers: portalUsers.map((entry) => ({
          ...
          access: {
            activeSessions:
              sessionsByUserId.get(Number(entry.user_id || 0))?.map((session) => ({
            ...
            notificationPreferences: preferenceByUserId.get(Number(entry.user_id || 0))
              ? {
                  caseActivityAlerts: Boolean(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.case_activity_alerts
                  ),
                  emailUpdates: Boolean(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.email_updates
                  ),
                  invoiceReminders: Boolean(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.invoice_reminders
                  ),
                  productAnnouncements: Boolean(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.product_announcements
                  ),
                  smsAlerts: Boolean(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.sms_alerts
                  ),
                  updatedAt: fromMysqlDateTime(
                    preferenceByUserId.get(Number(entry.user_id || 0))?.updated_at
                  ),
                }
              : null,
            securityEvents:
              securityEventsByUserId
                .get(Number(entry.user_id || 0))
                ?.map((securityEvent) => ({
  ...
  async updatePortalUserAccess(
    ...
    return withConnection(getMysqlPool(), async (connection) => {
      ...
      const contact = await selectOne<RowDataPacket>(
        connection,
        `SELECT cac.user_id, cac.client_account_id
         FROM client_account_contacts cac
         ...
         WHERE ca.public_id = ?
           ...
           AND cac.archived_at IS NULL
         LIMIT 1`,
        [clientAccountPublicId, userPublicId]
      );
      ...
      await connection.execute(
        ...
        [portalAccessEnabled ? 1 : 0, Number(contact.client_account_id), Number(contact.user_id)]
      );
      ...
      if (!portalAccessEnabled) {
        await connection.execute(
          `UPDATE user_sessions
           ...
           WHERE user_id = ?
             AND revoked_at IS NULL`,
          [timestamp, timestamp, Number(contact.user_id)]
        );
      }
  ...
  async forceSignOutPortalUser(actorUserId: number, clientAccountPublicId: string, userPublicId: string) {
    return withConnection(getMysqlPool(), async (connection) => {
      const timestamp = toMysqlDateTime(nowUtc());
      const contact = await selectOne<RowDataPacket>(
        connection,
