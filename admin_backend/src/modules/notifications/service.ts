...
export const adminNotificationService = {
  async listClientRecipients(connection: PoolConnection, clientAccountId: number) {
  ...
  async listInternalRecipients(connection: PoolConnection, userIds: number[]) {
    if (userIds.length === 0) {
      return [];
    }
    ...
    const rows = await selectAll<RecipientRow>(
      connection,
      `SELECT id AS user_id, public_id, email
       FROM users
  ...
  async listNotificationHistory(
    ...
    if (options.search?.trim()) {
      ...
      clauses.push(
        `(n.title LIKE ? ESCAPE '\\\\' OR n.body_text LIKE ? ESCAPE '\\\\' OR recipient.display_name LIKE ? ESCAPE '\\\\' OR COALESCE(${clientScopeExpression}, '') LIKE ? ESCAPE '\\\\')`
      );
      values.push(like, like, like, like);
    }

    const rows = await selectAll<NotificationHistoryRow>(
      connection,
      `SELECT
         n.public_id,
