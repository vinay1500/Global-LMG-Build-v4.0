...
import { adminJobQueue, type AdminAsyncJobRecord } from './jobQueue.js';

interface DueReminderRow extends RowDataPacket {
...
const claimDueReminders = async (limit = 50) =>
  withTransaction(getMysqlPool(), async (connection) => {
    ...
    const rows = await selectAll<DueReminderRow>(
      connection,
      `SELECT
         er.id,
         er.event_id,
    ...
    await connection.execute(
      ...
      [leaseToken, claimedAt, leaseExpiresAt, ...rows.map((row) => row.id)]
    );
...
const finalizeReminderSuccess = async (reminder: DueReminderRow, providerReference?: string | null) =>
  withTransaction(getMysqlPool(), async (connection) => {
    ...
    await executeResult(
      ...
      `UPDATE event_reminders
       ...
       WHERE id = ?
         AND lease_token = ?`,
      [sentAt, providerReference || null, reminder.id, reminder.lease_token]
    );
  });

const finalizeReminderFailure = async (reminder: DueReminderRow, error: unknown) =>
  withTransaction(getMysqlPool(), async (connection) => {
    const message = error instanceof Error ? error.message : String(error);
    const nextAttemptAt = toMysqlDateTime(
      new Date(Date.now() + Math.min(30 * 60_000, 30_000)).toISOString()
    );
  ...
  for (const job of jobs) {
    try {
      if (job.typeCode === 'event_lifecycle') {
        await processEventLifecycleJob(job.payload as {
      ...
      await adminJobQueue.failJob(job as AdminAsyncJobRecord, error);
