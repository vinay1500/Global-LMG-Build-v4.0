...
const seedBootstrapAdmin = async (connection: PoolConnection) => {
  const timestamp = toMysqlDateTime(nowUtc());
  const nameParts = splitDisplayName(env.ADMIN_BOOTSTRAP_NAME);
  const existingUser = await selectOne<RowDataPacket>(
    connection,
  ...
  if (!userId) {
    ...
    userId = Number((insert as { insertId: number }).insertId);
  } else {
    await connection.execute(
      `UPDATE users
       SET display_name = ?,
           ...
           archived_at = NULL
       WHERE id = ?`,
      [env.ADMIN_BOOTSTRAP_NAME, nameParts.firstName, nameParts.lastName, timestamp, timestamp, userId]
    );
  }

  const existingCredential = await selectOne<RowDataPacket>(
    connection,
  ...
  if (!existingCredential?.user_id) {
    ...
    await connection.execute(
      ...
      [userId, passwordHash, 'scrypt', timestamp, 1]
    );
  }
  ...
  await connection.execute(
    ...
    ON DUPLICATE KEY UPDATE
      ...
      is_active = VALUES(is_active),
      updated_at = VALUES(updated_at)`,
    [userId, env.ADMIN_BOOTSTRAP_ROLE_CODE, userId, null, null, 1, timestamp, timestamp]
  );
};
