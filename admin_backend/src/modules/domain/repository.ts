...
export class DomainRepository {
  ...
  public async createRefund(
    ...
    return withTransaction(this.pool, async (connection) => {
      ...
      if (invoiceId) {
        const paymentInvoiceMatch = await selectOne<RowDataPacket>(
          connection,
        ...
        if (!paymentInvoiceMatch?.invoice_id) {
          throw conflict(
            'refund_invoice_payment_mismatch',
            'The selected invoice is not linked to the payment being refunded.'
          );
        }
      }
  ...
  public async createAdminUser(actorUserPublicId: string, input: CreateAdminUserInput) {
    await this.initialize();
    ...
    return withTransaction(this.pool, async (connection) => {
      ...
      for (const roleCode of roleCodes) {
        await connection.execute(
          ...
          [userId, roleCode, actorUserId, timestamp, null, 1, timestamp, timestamp]
        );
      }
  ...
  public async resetAdminUserPassword(
    actorUserPublicId: string,
    userPublicId: string,
  ...
  public async updateAdminUserAccess(
    actorUserPublicId: string,
    userPublicId: string,
    ...
    return withTransaction(this.pool, async (connection) => {
      const actorUserId = await this.resolveUserId(connection, actorUserPublicId);
      const targetUserId = await this.resolveUserId(connection, userPublicId);
      const currentUser = await selectOne<RowDataPacket>(
        connection,
        `SELECT login_enabled, account_status_code, archived_at
         FROM users
         WHERE id = ?
         LIMIT 1`,
        [targetUserId]
      ...
      const timestamp = toMysqlDateTime(nowUtc());
      const loginEnabled =
        typeof input.loginEnabled === 'boolean'
          ? input.loginEnabled
      ...
      await connection.execute(
        `UPDATE users
         SET login_enabled = ?,
             ...
             row_version = row_version + 1
         WHERE id = ?`,
        [loginEnabled ? 1 : 0, accountStatusCode, archivedAt, timestamp, targetUserId]
      );
