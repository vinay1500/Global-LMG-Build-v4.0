...
export const adminTaskService = {
  ...
  async createTask(actorUserId: number, actorRoleCode: string, input: z.infer<typeof taskInputSchema>) {
    ...
    return withTransaction(getMysqlPool(), async (connection) => {
      ...
      for (const assigneeUserPublicId of payload.assigneeUserIds || []) {
        const assignee = await selectOne<RowDataPacket>(
          connection,
        ...
        await connection.execute(
          `INSERT INTO admin_task_assignees (
            task_id, user_id, assigned_by_user_id, assigned_at, status_code
          ) VALUES (?, ?, ?, ?, ?)`,
          [taskId, Number(assignee.id), actorUserId, timestamp, 'active']
        );
      }
      ...
      const row = await selectOne<RowDataPacket>(
        connection,
        'SELECT public_id FROM admin_tasks WHERE id = ? LIMIT 1',
        [taskId]
      );
  ...
  async updateTaskStatus(
    ...
    return withTransaction(getMysqlPool(), async (connection) => {
      const task = await selectOne<RowDataPacket>(
        connection,
        'SELECT id FROM admin_tasks WHERE public_id = ? AND archived_at IS NULL LIMIT 1',
        [taskPublicId]
      );
      ...
      await connection.execute(
        `UPDATE admin_tasks
         ...
         WHERE id = ?`,
        [payload.statusCode, timestamp, Number(task.id)]
      );
  ...
  async addTaskComment(
    ...
    actorRoleCode: string,
    taskPublicId: string,
    input: z.infer<typeof taskCommentSchema>
  ) {
    ...
    return withTransaction(getMysqlPool(), async (connection) => {
      ...
      const insert = await executeResult(
        ...
        [createPublicId(), Number(task.id), actorUserId, payload.bodyText, timestamp, timestamp]
      );
  ...
  async listTaskComments(taskPublicId: string) {
    return withTransaction(getMysqlPool(), async (connection) => {
      const task = await selectOne<RowDataPacket>(
        connection,
        'SELECT id FROM admin_tasks WHERE public_id = ? AND archived_at IS NULL LIMIT 1',
