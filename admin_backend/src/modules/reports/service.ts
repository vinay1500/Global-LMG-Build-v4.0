...
interface StaleMatterRow extends RowDataPacket {
  client_display_name: string;
...
interface PendingReminderRow extends RowDataPacket {
  client_display_name: string;
...
interface FailedJobRow extends RowDataPacket {
...
interface WaitingThreadRow extends RowDataPacket {
  client_display_name: string;
...
export const adminReportService = {
  async getOverview() {
    return withConnection(getMysqlPool(), async (connection) => {
      const [
        ...
        selectOne<CountRow>(
          connection,
          'SELECT COUNT(*) AS count_value FROM client_accounts WHERE archived_at IS NULL'
        ),
        selectOne<CountRow>(
          connection,
          'SELECT COUNT(*) AS count_value FROM matters WHERE archived_at IS NULL'
        ),
        selectOne<CountRow>(
          connection,
          'SELECT COUNT(*) AS count_value FROM conversation_threads WHERE archived_at IS NULL'
        ),
        selectOne<CountRow>(
          connection,
          'SELECT COUNT(*) AS count_value FROM documents WHERE archived_at IS NULL'
        ),
        selectOne<CountRow>(
          connection,
      ...
      const [matterStages, invoiceStatuses, eventStatuses, onboardingStatuses] = await Promise.all([
        selectAll<GroupRow>(
          connection,
          `SELECT current_stage_code AS group_code, COUNT(*) AS count_value
           FROM matters
           WHERE archived_at IS NULL
        ...
        selectAll<GroupRow>(
          connection,
          `SELECT status_code AS group_code, COUNT(*) AS count_value
           FROM invoices
           WHERE archived_at IS NULL
        ...
        selectAll<GroupRow>(
          connection,
          `SELECT onboarding_status_code AS group_code, COUNT(*) AS count_value
  ...
  async downloadOverviewCsv() {
    ...
    const rows = [
      'section,code,count',
      ...Object.entries(overview.counts).map(([code, count]) =>
        ['counts', code, count].map(escapeCsvValue).join(',')
      ),
      ...overview.matterStages.map((entry) =>
        ['matterStages', entry.code, entry.count].map(escapeCsvValue).join(',')
      ),
      ...overview.invoiceStatuses.map((entry) =>
        ['invoiceStatuses', entry.code, entry.count].map(escapeCsvValue).join(',')
      ),
      ...overview.eventStatuses.map((entry) =>
        ['eventStatuses', entry.code, entry.count].map(escapeCsvValue).join(',')
      ),
      ...overview.onboardingStatuses.map((entry) =>
        ['onboardingStatuses', entry.code, entry.count].map(escapeCsvValue).join(',')
      ),
    ];
  ...
  async getDrilldowns() {
    return withConnection(getMysqlPool(), async (connection) => {
      const [overdueInvoices, staleMatters, pendingReminders, failedJobs, waitingThreads] =
        await Promise.all([
          selectAll<OverdueInvoiceRow>(
            connection,
            `SELECT
               i.public_id,
               i.invoice_number,
  ...
  async retryAsyncJob(jobPublicId: string) {
      ...
      const result = await executeResult(
        ...
        `UPDATE admin_async_jobs
         SET status_code = 'retry',
             ...
             updated_at = ?
         WHERE public_id = ?
           AND status_code IN ('failed', 'retry')`,
        [timestamp, timestamp, jobPublicId]
      );
  ...
  async retryReminder(reminderId: number) {
    return withTransaction(getMysqlPool(), async (connection) => {
      const timestamp = toMysqlDateTime(nowUtc());
      const result = await executeResult(
        connection,
        `UPDATE event_reminders
         ...
         WHERE id = ?
           AND sent_at IS NULL
           AND delivery_status_code IN ('failed', 'retry')`,
