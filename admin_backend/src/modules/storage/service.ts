...
const resolveAdminOwnerContext = async (
  connection: PoolConnection,
  ownerPublicId: string,
  relatedEntityType?: string,
  relatedEntityId?: string
) => {
  const ownerUser = await selectOne<RowDataPacket>(
    connection,
  ...
  if (relatedEntityType === 'thread') {
    const thread = await selectOne<RowDataPacket>(
      connection,
      'SELECT client_account_id FROM conversation_threads WHERE public_id = ? LIMIT 1',
      [relatedEntityId]
    );
  ...
  if (relatedEntityType === 'matter') {
    const matter = await selectOne<RowDataPacket>(
      connection,
      'SELECT client_account_id FROM matters WHERE public_id = ? LIMIT 1',
      [relatedEntityId]
    );
  ...
  if (relatedEntityType === 'invoice') {
    const invoice = await selectOne<RowDataPacket>(
      connection,
      'SELECT client_account_id FROM invoices WHERE public_id = ? LIMIT 1',
      [relatedEntityId]
    );
...
const linkStoredDocument = async (
  ...
  if (input.relatedEntityType === 'client-account') {
    const clientRow = await selectOne<RowDataPacket>(
      connection,
      'SELECT id FROM client_accounts WHERE public_id = ? LIMIT 1',
      [input.relatedEntityId]
    );
    ...
    if (clientRow?.id) {
      await connection.execute(
        `UPDATE documents
         ...
         WHERE id = ?`,
        [Number(clientRow.id), input.documentId]
      );
    }
  }
};
...
export const documentStorageService = {
  ...
  async storeUploadContent(ownerAccountId: string, uploadId: string, content: Buffer) {
    ...
    await withTransaction(getMysqlPool(), async (connection) => {
      const ownerContext =
        (await resolveOwnerContext(connection, ownerAccountId)) ||
        (await resolveAdminOwnerContext(
          connection,
          ownerAccountId,
