...
export interface SchemaMigrationDefinition {
  ...
  description: string;
  id: string;
  run?: (connection: PoolConnection) => Promise<void>;
  statements?: string[];
}
...
const informationSchemaExists = async (
  connection: PoolConnection,
  query: string,
  values: Array<string>
) => {
  const [rows] = await connection.query<InformationSchemaCountRow[]>(query, values);
...
export const NORMALIZED_MIGRATIONS: SchemaMigrationDefinition[] = [
  ...
  },
  {
    ...
    run: async (connection) => {
      ...
      await connection.query(
        `CREATE TABLE IF NOT EXISTS admin_async_jobs (
          id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
          public_id CHAR(26) NOT NULL,
